...
def display_talks(day, hour):
    ...
    return {'talks': Talk.objects.filter(date__contains=day.date).filter(date__contains=hour)}

@register.inclusion_tag('tags/display_talks.html')
def display_talks_zone(day, hour, zone):
    """ Show fisl talks given a day, hour and zone """

    return {'talks': Talk.objects.filter(zone=zone, date__contains=day.date).filter(date__contains=hour)}

@register.inclusion_tag('tags/display_talks.html')
def display_talks_room(day, hour, room):
    """ Show fisl talks given a day, hour and room """

    return {'talks': Talk.objects.filter(room=room, date__contains=day.date).filter(date__contains=hour)}
